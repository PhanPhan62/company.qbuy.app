        ...
        <div class="collapse navbar-collapse">
            <ul class="navbar-nav navbar-left display-flex align-items-center">
                                ...
                                if ($item->item_type == "category"):
                                    ...
                                    if (!empty($category)): ?>
                                        <li class="nav-item nav-item-category nav-item-category-<?= $category->id; ?>" data-category-id="<?= $category->id; ?>">
                                            <a style="font-size:16px"style="font-size:16px" href="<?= generateCategoryURL($category); ?>" class="nav-link" data-toggle="dropdown" role="button" aria-expanded="false">
                                                <?= esc($category->name); ?><i class="icon-arrow-down"></i>
                                            </a>
                                        </li>
                                        <?php 
                                        if (!empty($subLinks)) {
                                    ...
                                    if (!empty($subLinks)): ?>
                                        <li class="nav-item dropdown">
                                            <a style="font-size:16px"style="font-size:16px" class="nav-link" href="<?= generateMenuItemURL($item, $baseCategories); ?>">
                                                <?= esc($item->item_name); ?><i class="icon-arrow-down"></i>
                                            </a>
                                            <ul class="dropdown-menu nav-dropdown-menu">
                                                <?php foreach ($subLinks as $subItem):
                                                    if ($subItem->item_visibility == 1): ?>
                                                        <li>
                                                            <a style="font-size:16px"href="<?= generateMenuItemURL($subItem, $baseCategories); ?>" class="dropdown-item">
                                    ...
                                    <?php else: ?>
                                        <li class="nav-item <?= uri_string() == $item->item_slug ? 'active' : ''; ?>">
                                            <a style="font-size:16px"href="<?= generateMenuItemURL($item, $baseCategories); ?>" class="nav-link">
                                                <?= esc($item->item_name); ?>
                                            </a>
                                        </li>
                                    <?php endif;
                ...
                if ($totalItem >= $menuLimit): ?>
                    <li class="nav-item dropdown">
                        <a style="font-size:16px"style="font-size:16px" class="nav-link" href="#"><?= trans("more"); ?><i class="icon-arrow-down"></i></a>
                        <ul class="dropdown-menu nav-dropdown-menu">
                            <?php
                            $i = 1;
                            if (!empty($baseMenuLinks)):
                                foreach ($baseMenuLinks as $item):
                                    if ($item->item_visibility == 1 && $item->item_location == "main" && $item->item_parent_id == "0"):
                                        if ($i >= $menuLimit):
                                            ...
                                            if (!empty($subLinks)): ?>
                                                <li>
                                                    <a style="font-size:16px"href="<?= generateMenuItemURL($item, $baseCategories); ?>" class="dropdown-item display-flex align-items-center">
                                                        <?= esc($item->item_name); ?><i class="icon-arrow-down"></i>
                                                    </a>
                                            ...
                                            <?php else: ?>
                                                <li>
                                                    <a style="font-size:16px"href="<?= generateMenuItemURL($item, $baseCategories); ?>" class="dropdown-item">
                                                        <?= esc($item->item_name); ?>
                                                    </a>
                                                </li>
                                            <?php endif;
...
<div class="container-xl">
    <div class="mega-menu-container">
        <?php if (!empty($itemsMegaMenu)): ?>
            <?php foreach ($itemsMegaMenu as $itemMegaMenu): ?>
                <?php if (!empty($itemMegaMenu['category'])): ?>
                    <?php 
                            ...
                            <div class="row">
                                ...
                                ?>
                                    <div class="col-3 menu-left">
                                        <a style="font-size:16px"href="<?= generateCategoryURL($category); ?>" data-category-filter="all"
                                            class="link-sub-category link-sub-category-all active"><?= trans("all"); ?></a>
                                        <?php foreach ($subCategories as $subCategory): ?>
                                            <?php if ($subCategory->show_on_menu == 1): ?>
                                                <a style="font-size:16px"href="<?= generateCategoryURL($subCategory); ?>"
                                                    data-category-filter="<?= esc($subCategory->id); ?>"
                                                    class="link-sub-category"><?= esc($subCategory->name); ?></a>
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                                    </div>
                                <?php endif; ?>
                                                ...
                                                ?>
                                                            <div class="col-sm-2 menu-post-item<?= $postLimit == 5 ? ' width20' : ' width25'; ?>">
                                                                <?php if (checkPostImg($item)): ?>
                                                                    <div class="image">
                                                                        <a style="font-size:16px"href="<?= generatePostURL($item); ?>" <?php postURLNewTab($item); ?>>
                                                                            <img src="<?= IMG_BASE64_1x1; ?>"
                                                                                data-src="<?= getPostImage($item, 'mid'); ?>"
                                                                                alt="<?= esc($item->title); ?>" class="img-fluid lazyload"
                                                                                width="232" height="140" />
                                                                            <?php getMediaIcon($item, 'media-icon-sm'); ?>
                                                                        </a>
                                                                    </div>
                                                                <?php endif; ?>
                                                                <h3 class="title"><a style="font-size:16px"href="<?= generatePostURL($item); ?>" <?php postURLNewTab($item); ?>>
                                                                    <?= esc(characterLimiter($item->title, 45, '...')); ?></a>
                                                                </h3>
