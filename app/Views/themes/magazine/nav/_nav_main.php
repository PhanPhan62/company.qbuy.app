...
<div class="navbar navbar-expand-md nav-main">
    <nav class="container-xl">
        <a href="<?= langBaseUrl(); ?>" class="navbar-brand p-0">
            <img src="<?= $darkMode == 1 ? getLogoFooter() : getLogo(); ?>" alt="logo" class="logo" width="180"
                height="52">
        </a>
...
if (!empty($category)): ?>
                <li class="nav-item nav-item-category nav-item-category-<?= $category->id; ?>"
                    data-category-id="<?= $category->id; ?>">
                    <a href="<?= generateCategoryURL($category); ?>" class="nav-link" data-toggle="dropdown"
                        role="button" aria-expanded="false"><?= esc($category->name); ?><i
...
if (!empty($subLinks)): ?>
                <li class="nav-item dropdown">
                    <a class="nav-link"
                        href="<?= generateMenuItemURL($item, $baseCategories); ?>"><?= esc($item->item_name); ?><i
                            class="icon-arrow-down"></i></a>
                    <ul class="dropdown-menu nav-dropdown-menu">
                        <?php foreach ($subLinks as $subItem):
if ($subItem->item_visibility == 1): ?>
                        <li><a href="<?= generateMenuItemURL($subItem, $baseCategories); ?>"
                                class="dropdown-item"><?= esc($subItem->item_name); ?></a></li>
                        <?php endif;
endforeach; ?>
                    </ul>
                </li>
                <?php else: ?>
                <li class="nav-item <?= uri_string() == $item->item_slug ? 'active' : ''; ?>"><a
...
if (!empty($subLinks)): ?>
                        <li>
                            <a href="<?= generateMenuItemURL($item, $baseCategories); ?>"
                                class="dropdown-item display-flex align-items-center"><?= esc($item->item_name); ?><i
                                    class="icon-arrow-down"></i></a>
...
endforeach; ?>
                        ...
                        <?php else: ?>
                        <li><a href="<?= generateMenuItemURL($item, $baseCategories); ?>"
                                class="dropdown-item"><?= esc($item->item_name); ?></a></li>
                        <?php endif;
endif;
$i++;
endif;
                    ...
                    <div class="search-form">
                        <form action="<?= generateURL('search'); ?>" method="get" id="search_validate">
                            <input type="text" name="q" maxlength="300" pattern=".*\S+.*"
                                class="form-control form-input" placeholder="<?= trans("placeholder_search"); ?>"
                                <?= $rtl == true ? 'dir="rtl"' : ''; ?> required>
...
if ($i < $postLimit): ?>
                                    <div
                                        class="col-sm-2 menu-post-item<?= $postLimit == 5 ? ' width20' : ' width25'; ?>">
                                        ...
                                        <?php endif; ?>
                                        <h3 class="title"><a href="<?= generatePostURL($item); ?>"
                                                <?php postURLNewTab($item); ?>><?= esc(characterLimiter($item->title, 45, '...')); ?></a>
                                        </h3>
