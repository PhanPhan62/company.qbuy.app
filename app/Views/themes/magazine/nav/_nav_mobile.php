                ...
                <form action="<?= generateURL('search'); ?>" method="get" id="search_validate">
                    <div class="display-flex align-items-center">
                        <input type="text" name="q" maxlength="300" pattern=".*\S+.*" class="form-control form-input"
                            placeholder="<?= trans("placeholder_search"); ?>" <?= $rtl == true ? 'dir="rtl"' : ''; ?>
                            required>
...
if (checkUserPermission('add_post')): ?>
                <button class="btn btn-md btn-block btn-custom btn-mobile-nav btn-mobile-nav-add close-menu-click"
                ...
                <div class="row">
                    <div class="col-6">
                        <a href="#" data-bs-toggle="modal" data-bs-target="#modalLogin"
                            class="btn btn-md btn-custom close-menu-click btn_open_login_modal btn-auth-mobile">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="nav-item dropdown profile-dropdown-mobile">
                    <a href="#" class="nav-link dropdown-toggle nav-link-mobile-profile" data-bs-toggle="dropdown"
                        aria-expanded="false">
                        <img src="<?= getUserAvatar(user()->avatar); ?>" alt="<?= esc(user()->username); ?>" width="40"
                            height="40">
                    ...
                    <ul class="dropdown-menu menu-sub-items">
                        ...
                        <?php endif; ?>
                        <li><a href="<?= generateProfileURL(user()->slug); ?>"
                                class="dropdown-item"><?= trans("profile"); ?></a></li>
                        <?php if (user()->reward_system_enabled == 1): ?>
                        <li><a href="<?= generateURL('earnings'); ?>"
                                class="dropdown-item"><?= trans("earnings"); ?>&nbsp;(<strong
                                    class="lbl-earnings"><?= priceFormatted(user()->balance); ?></strong>)</a></li>
                        <?php endif; ?>
                        <li><a href="<?= generateURL('reading_list'); ?>"
                                class="dropdown-item"><?= trans("reading_list"); ?></a></li>
                        <li><a href="<?= generateURL('settings'); ?>"
                                class="dropdown-item"><?= trans("settings"); ?></a></li>
                        <li><a href="<?= generateURL('logout'); ?>" class="dropdown-item"><?= trans("logout"); ?></a>
                        </li>
                    </ul>
                </div>
            </div>
...
if (!empty($subLinks)): ?>
                    <li class="nav-item dropdown">
                        <a href="#" class="dropdown-toggle d-flex justify-content-between nav-link"
                        ...
                        <ul class="dropdown-menu menu-sub-items">
                            ...
                            <li class="nav-item"><a href="<?= generateMenuItemURL($item, $baseCategories); ?>"
                                    class="nav-link"><?= trans("all"); ?></a></li>
                            <?php endif;
foreach ($subLinks as $sub):
if ($sub->item_visibility == 1):?>
                            <li class="nav-item"><a href="<?= generateMenuItemURL($sub, $baseCategories); ?>"
                                    class="nav-link"><?= esc($sub->item_name) ?></a></li>
                            <?php endif;
endforeach; ?>
                        </ul>
                    </li>
...
$langURL = base_url();
            ...
            <div class="col-4">
                <form action="<?= base_url('switch-dark-mode'); ?>" method="post">
                    ...
                    <?php if ($darkMode == 1): ?>
                    <button type="submit" name="theme_mode" value="light"
                        class="btn btn-md btn-default btn-switch-mode-mobile" aria-label="dark-mode">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor"
                            viewBox="0 0 16 16">
                            <path
                    ...
                    <?php else: ?>
                    <button type="submit" name="theme_mode" value="dark"
                        class="btn btn-md btn-default btn-switch-mode-mobile" aria-label="dark-mode">
