...
.modal-newsletter .btn {
  background-color: <?=$activeTheme->block_color;
  ?>
}
...
.comment-section .nav-tabs {
  border-bottom: 2px solid <?=$activeTheme->block_color;
  ?>;
}
...
.list-footer-categories li a:hover {
  color: <?=$activeTheme->theme_color;
  ?>;
}
...
.spinner>div,
.search-form button {
  background-color: <?=$activeTheme->theme_color;
  ?> !important;
}
...
.btn-custom {
  background-color: <?=$activeTheme->theme_color;
  ?>;
  border-color: <?=$activeTheme->theme_color;
  ?>;
}

::selection {
  background: <?=$activeTheme->theme_color;
  ?> !important;
  color: #fff;
}
...
.comments .comments-title {
  border-bottom: 2px solid <?=$activeTheme->theme_color;
  ?>;
}

.comment-loader-container .loader,
.sub-comment-loader-container .loader {
  border-top: 5px solid <?=$activeTheme->theme_color;
  ?>;
}
...
.newsletter .newsletter-button {
  background-color: <?=$activeTheme->theme_color;
  ?>;
  border: 1px solid <?=$activeTheme->theme_color;
  ?>;
}
...
.search-form,
.dropdown-more {
  border-top: 3px solid <?=$activeTheme->theme_color;
  ?>;
}
...
<?php if ( !empty($adSpaces)): foreach ($adSpaces as $item): if ( !empty($item->desktop_width) && !empty($item->desktop_height)): echo '.bn-ds-'.$item->id. '{width: '. $item->desktop_width . 'px; height: '. $item->desktop_height . 'px;}';
