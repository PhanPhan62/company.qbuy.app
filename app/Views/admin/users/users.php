                      ...
                      <ul class="dropdown-menu options-dropdown">
                        ...
                        <li>
                          <button type="button" class="btn-list-button" data-toggle="modal" data-target="#myModal"
                            onclick="$('#modal_user_id').val('<?= $user->id; ?>');">
                        ...
                        <li>
                          <button type="submit" name="submit" value="reward_system" class="btn-list-button"><i
                              class="fa fa-money option-icon"></i><?= trans('disable_reward_system'); ?></button>
                        </li>
                        <?php else: ?>
                        <li>
                          <button type="submit" name="submit" value="reward_system" class="btn-list-button"><i
                              class="fa fa-money option-icon"></i><?= trans('enable_reward_system'); ?></button>
                        </li>
                        <?php endif;
                                                    if ($user->email_status != 1): ?>
                        <li>
                          <button type="submit" name="submit" value="confirm_email" class="btn-list-button"><i
                              class="fa fa-check option-icon"></i><?= trans('confirm_user_email'); ?></button>
                        </li>
                        <?php endif;
                                                    if ($user->status == "1"): ?>
                        <li>
                          <button type="submit" name="submit" value="ban_user" class="btn-list-button"><i
                              class="fa fa-stop-circle option-icon"></i><?= trans('ban_user'); ?></button>
                        </li>
                        <?php else: ?>
                        <li>
                          <button type="submit" name="submit" value="ban_user" class="btn-list-button"><i
                              class="fa fa-stop-circle option-icon"></i><?= trans('remove_ban'); ?></button>
                        </li>
                        <?php endif; ?>
                        <li>
                          <a href="<?= adminUrl('edit-user/' . $user->id); ?>"><i
                              class="fa fa-edit option-icon"></i><?= trans('edit'); ?></a>
                        </li>
                        <li>
                          <a href="javascript:void(0)"
                            onclick="deleteItem('AdminController/deleteUserPost','<?= $user->id; ?>','<?= clrQuotes(trans("confirm_user")); ?>');"><i
              ...
              <?php $roles = $authModel->getRolesPermissions();
                            if (!empty($roles)):
                                foreach ($roles as $role):
                                    $roleName = parseSerializedNameArray($role->role_name, $activeLang->id); ?>
              <div class="col-sm-3">
                <input type="radio" name="role" value="<?= esc($role->role); ?>" id="role_<?= esc($role->role); ?>"
                  class="square-purple" required>&nbsp;&nbsp;
                <label for="role_<?= esc($role->role); ?>"
