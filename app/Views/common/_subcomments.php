    ...
    <ul class="comment-list">
      ...
      <li>
        <div class="left">
          ...
          <a href="<?= generateProfileURL($subComment->user_slug); ?>"><img
              src="<?= getUserAvatar($subComment->user_avatar); ?>" alt="<?= esc($subComment->name); ?>"></a>
          <?php else: ?>
        ...
        <div class="right">
          <div class="row-custom">
            ...
            <a href="<?= generateProfileURL($subComment->user_slug); ?>"
              class="username"><?= esc($subComment->name); ?></a>
            <?php else: ?>
            ...
            <div class="d-flex justify-content-start align-items-center comment-meta">
              ...
              <div class="item item-like">
                <a href="javascript:void(0)"
                  class="btn-comment-like<?= isCommentVoted($subComment->id) ? ' comment-liked' : ''; ?><?= checkCommentOwner($subComment) ? ' comment-own' : ''; ?>"
                  ...
                  <?= trans("like"); ?>&nbsp;(<span
                    id="lbl_comment_like_count_<?= $subComment->id; ?>"><?= $subComment->like_count; ?></span>)</a>
              </div>
              <?php if (authCheck()):
                                            if ($subComment->user_id == user()->id || user()->role == 'admin'): ?>
              <div class="item item-delete">
                <a href="javascript:void(0)" class="btn-delete-comment"
                  onclick="deleteComment('<?= $subComment->id; ?>','<?= $post->id; ?>','<?= clrQuotes(trans("confirm_comment")); ?>');">&nbsp;<i
